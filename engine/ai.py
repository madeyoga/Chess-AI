    ...
    def get_decision(self, player):
        ...
        for movement in possible_moves:
            for piece_move in movement['available_moves']:
                ...
                new_board = self.board.make_move(movement['index'], piece_move)

                if player == self.player1:
                    score = self.minimizer(new_board, 0)
                    ...
                    score = self.maximizer(new_board, 0)
                    if score < best_score:
                        best_score = score
                        best_move = {'index': movement['index'], 'move': piece_move}
                        print('best', best_move, best_score)
    ...
    def maximizer(self, board, threshold):
        ...
        for movement in possible_moves:
            for piece_move in movement['available_moves']:
                ...
                score = self.minimizer(new_board, threshold + 1)
    ...
    def minimizer(self, board, threshold):
        ...
        for movement in possible_moves:    
            for piece_move in movement['available_moves']:
                ...
                score = self.maximizer(new_board, threshold + 1)
